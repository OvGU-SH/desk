...
#' }
#'
#' @details Let \code{X} be an object generated by \code{r.sample()} then \code{plot(X, ...)} accepts the following arguments:
#' \tabular{ll}{
...
#' out = r.sample(x, true.par = c(2,1,4), rep = 10)
#'
#' ## Extract some data
...
#' out = r.sample(x, true.par = c(2,1,4), rep = 10, xnew = cbind(x1 = c(1.5,6,7), x2 = c(1,3,5.5)))
...
#' out = r.sample(x, true.par = c(2,1,4), rep = 1000)
#' out$outside.ci
#'
#' ## Illustrate omitted variable bias
#' out.unbiased = r.sample(x, true.par = c(2,1,4))
#' mean(out.unbiased$coef["beta1",]) # approx. equal to beta1 = 1
#' out.biased = r.sample(x, true.par = c(2,1,4), omit = 2) # omit x2
...
#' out = r.sample(X, true.par = c(2,1,4), rep = 1) # Simulate a regression
#' out$vcov.coef
#'
#' ## Illustrate confidence intervals
#' out = r.sample(c(10, 20, 30,50), true.par = c(0.2,0.13), rep = 10, seed = 12)
...
#' out = r.sample(c(10,30,50), true.par = c(0.2,0.13), rep = 200)
