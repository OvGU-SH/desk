...
#'
#' @param mod estimated linear model object or formula. If only a model formula is passed then the \code{data} argument must be specified.
#' @param data if \code{mod} is a formula then the corresponding data frame has to be specified.
#' @param split partitions the data set into two groups. If <= 1 then \code{split} is a percentage value such that \code{T*split} observations are in the first partition. If \code{split} >= 1 it is interpreted as the index of the partitioning observation, i.e. the number of observations in the first group.
#' @param omit.obs the number of central observations to be omitted. Might increase the power of the test. If <= 1 then \code{split} is the percentage value of all observations, otherwise it is interpreted as absolute number.
#' @param ah character string specifying the type of the alternative hypothesis: \code{"increasing"} (variance increases from group 1 to group 2), \code{"decreasing"} (variance decreases from group 1 to group 2), \code{"unequal"} (variances are unequal between the groups). The default is to test for increasing variances.
#' @param order.by either a vector \code{z} or a formula with a single explanatory variable like \code{~ z}. The observations in the model are ordered by the size of \code{z}. If set to \code{NULL} (the default) the observations are assumed to be ordered.
